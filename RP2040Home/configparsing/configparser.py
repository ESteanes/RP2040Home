    ...
    def validate_config(self, config: any, allowed_pins: list[str], output_config: list[Output]):
        for x in config['digital_outputs']:
            if x['pin'] not in allowed_pins:
                raise AttributeError("Output on " + str(x['pin']) + " is not allowed, the only allowed GPIO pins are: " + ",".join(map(str,allowed_pins)))
        ...
        if len(output_config) != len(config['digital_outputs']):
            raise AttributeError("One or more outputs has been misconfigured, the only allowed GPIO pins are: " + ",".join(map(str,allowed_pins)))
        try:
            print(config)
